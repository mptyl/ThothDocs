...
- **Monitoraggio**: Configurare Logfire per il monitoraggio dell'applicazione (vedi [monitoring](../3-user_manual/3.4-logging/3.4.1-logfire.md)
...
Su Logfire non vengono mai inviati dati sul contenuto dei database interrogati, ma, se non si vuole utilizzare il servizio, basta lasciare vuota la chiave e Logfire non verrà utilizzato. In questo caso rimarranno solo i file di log salvati in locale [(vedi logs)](../3-user_manual/3.4-logging/3.4.2-log_management.md). Non si avrà però lo stesso livello di granularità e di insight fornita da Logfire.
