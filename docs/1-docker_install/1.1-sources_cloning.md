...
- **Database utilizzati**: Indicare i database che si intende utilizzare. E' possibile gestire database di tipo SQLite, MariaDB, MySql, PostgreSQL, Informix e SQL Server;
...
- **Monitoraggio**: Configurare `Logfire` per il monitoraggio dell'applicazione (vedi [monitoring](../4-user_manual/4.3-logging/4.3.1-logfire.md){:target="_blank"}, indicando se si intende utilizzare il servizio di monitoraggio tramite `Logfire` e, in quel caso, il token di accesso;
...
!!! note "Provider LLM consigliato per iniziare"
    ...
    Per una rapida prova di ThothAI, si consiglia di utilizzare **OpenRouter** che fornisce accesso a centinaia di modelli LLM con una singola API key e include anche modelli gratuiti. L'installazione Docker di ThothAI prevede lla creazione di un ambiente **demo** già preconfigurato per utilizzare OpenRouter come provider e california_schools come database su cui esercitarsi.
